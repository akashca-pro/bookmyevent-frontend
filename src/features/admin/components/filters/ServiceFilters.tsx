...
const ServiceFiltersComponent = () => {
    ...
    useEffect(() => {
        const handler = setTimeout(() => setDebouncedMinPrice(minPrice), 500);
        return () => clearTimeout(handler);
    }, [minPrice]);
    ...
    const handleDistrictChange = useCallback((val: string) => {
        setSearchParams(prev => {
            const next = new URLSearchParams(prev);
            if (val && val !== "all") next.set("district", val);
    ...
    const hasFilters = useMemo(() => {
        return searchParams.toString() && searchParams.toString() !== "page=1";
    }, [searchParams]);
...
export const ServiceFilters = memo(ServiceFiltersComponent);
